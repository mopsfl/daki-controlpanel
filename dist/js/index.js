    ...
    !function(e, t) {
      ...
      !function() {
        ...
        function V2(e3) {
          ...
          return n3 != T2 && 9 === n3.nodeType && n3.documentElement && (r2 = (T2 = n3).documentElement, C2 = !ce.isXMLDoc(T2), i2 = r2.matches || r2.webkitMatchesSelector || r2.msMatchesSelector, r2.msMatchesSelector && ye != T2 && (t3 = T2.defaultView) && t3.top !== t3 && t3.addEventListener("unload", M3), le.getById = $2(function(e4) {
...
// modules/Panel.ts
var Panel_default = {
  SetVisible(_setup_panel2, state) {
    _setup_panel2.toggleClass("hide", !state);
  }
};
...
var Request_default = {
  async new(url, method, headers, body, auth) {
    ...
    if (auth)
      headers["Authorization"] = Request_default.CreateBearerAuthObject(auth);
...
// modules/Api.ts
var apiKey = void 0;
var DAKI_API_URL = "https://portal.daki.cc/api/";
var DAKI_API_ENDPOINTS = {
...
var _servers_panel = jQuery(".servers-panel");
var Api_default = {
  Connect(_apiKey) {
    if (apiKey !== void 0)
      return console.warn("Api already connected.");
    apiKey = _apiKey;
    Request_default.new(DAKI_API_ENDPOINTS.ListServers, "GET", {}, void 0, apiKey).then((_res) => _res.json()).then((res) => {
      if (res.errors) {
        res.errors.forEach((err) => {
          M.toast({
            html: Request_default.FormatApiError(err)
          });
        });
        Setup_default.ResetSetupPanel();
        return;
      }
      Panel_default.SetVisible(_setup_panel, false);
      Panel_default.SetVisible(_servers_panel, true);
      res.data?.forEach((_server) => {
        const [serverItem, serverItemName, serverItemBody, serverItemStatus] = Api_default.CreateServerItem(_server, _servers_panel);
        Api_default.GetServerResources(_server).then((resource) => {
  ...
  CreateServerItem(server, parent) {
    const serverItem = jQuery(".server-item-template").contents().clone(), serverItemName = serverItem.find(".server-item-name"), serverItemBody = serverItem.find(".server-item-body"), serverItemStatus = serverItem.find(".server-item-status");
  ...
  async GetServerResources(server) {
    return Request_default.new(DAKI_API_ENDPOINTS.Resources.replace("%UUID%", server.attributes.uuid), "GET", {}, void 0, apiKey).then((res) => res.json());
  }
};

// modules/Setup.ts
var _setup_panel = (0, import_jquery.default)(".setup-panel");
var _input_setup_api_key2 = (0, import_jquery.default)("#input_setup_api_key");
var _submit_input_setup2 = (0, import_jquery.default)("#submit_input_setup");
var Setup_default = {
  Init() {
    Panel_default.SetVisible(_setup_panel, true);
    _submit_input_setup2.on("click", () => {
      const _apiKeyInput = Setup_default.GetApiKeyInput().toString();
      if (Setup_default.CheckApiKeyInput(_apiKeyInput)) {
        _submit_input_setup2.prop("disabled", true).text("...");
        _input_setup_api_key2.prop("disabled", true);
        _setup_panel.addClass("blurbg");
        _input_setup_api_key2.val("");
        _input_setup_api_key2.prop("placeholder", "");
        Api_default.Connect(_apiKeyInput);
      } else {
        M.toast({
  ...
  ResetSetupPanel() {
    Panel_default.SetVisible(_setup_panel, true);
    Panel_default.SetVisible(_servers_panel, false);
    _submit_input_setup2.prop("disabled", false).text("CONNECT");
    _input_setup_api_key2.prop("disabled", false);
    _input_setup_api_key2.val("");
    _setup_panel.removeClass("blurbg");
    _input_setup_api_key2.prop("placeholder", "ptlc_umG...ar3");
  },
  GetApiKeyInput() {
    return _input_setup_api_key2.val();
  },
...
(0, import_jquery2.default)(() => {
  Setup_default.Init();
  window.modules = { Setup: Setup_default, Api: Api_default, Request: Request_default, Panel: Panel_default };
});
